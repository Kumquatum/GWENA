...
compare_modules = function(data_expr_list, net_list, cor_list = NULL, modules_list, ref = names(data_expr_list)[1], test = NULL,
                           cor_func = c("pearson", "spearman", "bicor", "other"), your_func = NULL,
                           n_perm = 10000, comparison_type = c("unpreserved", "preserved", "one or the other"), pvalue_th = 0.05,
                           n_threads = NULL, ...){
  # Basic checks
  ...
  if (!all(conditions %in% names(net_list))) stop("Names in net_list don't correspond to conditions.")
  if (!all(conditions %in% names(cor_list))) stop("Names in cor_list don't correspond to conditions.")
  ...
  if (ref != "cross comparison" && !(all(ref %in% conditions))) stop("ref must be a condition name, or a vector of it, matching conditions.")
  ...
  if (ref != "cross comparison") { # Checking adequation of content between conditions described in modules and reference choosen
    ...
    if (!all(conditions %in% names(modules_list))) stop("All conditions must have their modules described in modules_list when ref
                                                        is 'cross comparison'")
  }
  ...
  if (ref != "cross comparison" && exists("additionnal_modules_list")) {
    additionnal_modules_reformated <- sapply(additionnal_modules_list, function(cond){
      lapply(names(cond), function(x){
        setNames(rep(as.numeric(x), length(cond[[x]])), cond[[x]]) }) %>% unlist
    }, simplify = FALSE)
    for (reference in ref_set) {
      for (additional_test in names(additionnal_modules_list)) {
        preservation[[reference]][[additional_test]][["contingency"]] <- NetRep:::contingencyTable(additionnal_modules_reformated,
                                                                                                   additionnal_modules_reformated[[reference]] %>% table %>% names,
                                                                                                   additionnal_modules_reformated[[additional_test]] %>% names)
      }
    }
    # lapply(names(additionnal_modules_list), function(cond){
