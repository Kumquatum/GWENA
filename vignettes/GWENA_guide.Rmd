...
library(GWENA)
library(magrittr) # It is not mandatory, but in this guide we use the pipe `%>%` to ease lisibility
```
...
Similarly to gene differential expression, co-expression networks can be compared between condition to not only detect differences of expression but also differences in patterns of co-expression. These may indicate break of inhibition, inefficiency of a factor of transcription, etc.

Depending of your study, you might want to focus on the modules preserved between conditions, or the one unpreserved. Ex : if you're looking for housekeeping genes, you should test for the modules preserved; but if you're looking for the genes that drive a disease, you should test for the unpreserved ones.

Here in GWENA we'll use a comparison test based on random re-assignment of gene names inside module to see whether patterns inside modules change. This permutation test is repeated a large number of time to ensure the significance of the result.

In order to achieve comparison, we'll have to recompute the network and the module detection by condition. To avoid save cpu, memory and time, we'll switch the parameter `` to TRUE so the final object of the network building keep the similarity matrix used in the comparison test.
