...
eigen <- df %>%
  left_join(kuehne_expr_filtered %>% t %>% as.data.frame %>% 
              tibble::rownames_to_column(var = "gene")) %>%
...
plot_table <- df %>% 
  left_join(kuehne_expr_filtered %>% scale %>% as.data.frame %>% t %>% as.data.frame %>%
              tibble::rownames_to_column(var = "gene")) %>%
  tidyr::pivot_longer(c(-gene, -module), names_to = "sample", values_to = "expression") %>%
  left_join(eigen, c("module", "sample"), suffix = c("_gene", "_eigengene")) %>%
  group_by(gene_gene) %>%
  ...
  # group_by(module) %>%
  # group_by(sample) %>%
  # mutate(expression_scaled = scale(expression_gene)) %>%
...
ggplot2::ggplot(plot_table, ggplot2::aes(x=sample, y=expression, group=gene)) + 
  ...
  ggplot2::geom_line(ggplot2::aes(x = sample, y = expression_eigengene), color = "red") # fonctionne
 
# # Autre version. A retravailler ?
# eigen2 <- eigen %>% 
#   mutate(expression_neg = -expression) %>% 
#   bind_rows(., setNames(.[, c("sample", "gene", "module", "expression_neg", "expression")], c("sample", "gene", "module", "expression", "expression_neg")))
# 
# ggplot2::ggplot(plot_table, ggplot2::aes(x=sample, y=expression, group=gene)) + 
#   ggplot2::geom_line(alpha = 0.3) + 
#   ggplot2::facet_grid(cor_sign ~ module) + 
#   ggplot2::theme(axis.text.x=ggplot2::element_text(angle=90,vjust=0.5)) + ggplot2::theme(legend.position="none") +
#   ggplot2::geom_line(data=eigen2, ggplot2::aes(y = expression), color = "red")
#   
