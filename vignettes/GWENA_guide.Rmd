...
Since this operation tend to create some modules with highly similar expression profile (based on the [eigengene](FAQ.html/#what-is-an-eigengene) of each), these can be merged into one.
...
```{r bipartite_graph_merge, echo=FALSE}
g <- data.frame(before = detection$modules_premerge , after = detection$modules, stringsAsFactors = FALSE) %>% 
  distinct %>% 
  arrange(before) %>% 
  igraph::graph_from_data_frame()
igraph::V(g)$type <- lapply(igraph::V(g) %>% names %>% as.numeric, function(x) ifelse(x %in% unique(detection$modules), TRUE, FALSE)) %>% unlist
g %>% igraph::add_layout_(igraph::as_bipartite()) %>% plot()
```
...
# Facon WGCNA
eigen <- WGCNA::moduleEigengenes(kuehne_expr_filtered, detection$modules)$eigengenes %>%
  t %>%
  as.data.frame %>%
...
# Facon pca classique
mini <- data.frame(gene = names(detection$modules), module = detection$modules, stringsAsFactors = FALSE) %>%
  left_join(kuehne_expr_filtered %>% t %>% as.data.frame %>% tibble::rownames_to_column(var = "gene")) # temporaire, a virer ici ou dans le debut de df en dessous
  # left_join(kuehne_expr_filtered %>% apply(2, function(x) setNames(scale(x), names(x))) %>% t %>% as.data.frame %>% tibble::rownames_to_column(var = "gene")) # temporaire, a virer ici ou dans le debut de df en dessous
  
eigen2 <- split.data.frame(mini, mini$module) %>% 
  lapply(function(y) y[,c(-1,-2)] %>% t %>% prcomp(center = TRUE, scale.=TRUE) %>% .$x %>% .[, "PC1"] %>% scale) %>%
  # lapply(function(y) y[,c(-1,-2)] %>% t %>% prcomp %>% .$x %>% .[, "PC1"]) %>% 
  as.data.frame(check.names=FALSE) %>%
  tibble::rownames_to_column(var="sample") %>%
  mutate(gene = "eigengene") %>%
  tidyr::pivot_longer(c(-gene, -sample), names_to = "module", values_to = "expression")

df <- data.frame(gene = names(detection$modules), module = detection$modules, stringsAsFactors = FALSE) %>%
  # left_join(kuehne_expr_filtered %>% t %>% as.data.frame %>% tibble::rownames_to_column(var = "gene")) %>% #vanilla
  # left_join(kuehne_expr_filtered %>% t %>% as.data.frame %>% tibble::rownames_to_column(var = "gene") %>% apply(1, function(x) scale(x[, -1])) ) %>%
  left_join(kuehne_expr_filtered %>% apply(2, function(x) setNames(scale(x), names(x))) %>% t %>% as.data.frame %>% tibble::rownames_to_column(var = "gene")) %>%
  tidyr::pivot_longer(c(-gene, -module), names_to = "sample", values_to = "expression")

ggplot2::ggplot(df, ggplot2::aes(x=sample, y=expression, group=gene)) + 
  ...
  ggplot2::geom_line(data=eigen, ggplot2::aes(x = sample, y = expression), color = "red")
