...
threads_to_use <- 1
```
```{r load_other_os, message=FALSE, warning=FALSE, eval=!is_windows, echo=!is_windows}
library(GWENA)
library(magrittr) # Not mandatory, but in this tutorial we use the pipe `%>%` to ease readability.
...
S4Vectors::metadata(se_kuehne) <- list(
  ...
  overall_design = "Gene expression in epidermal skin samples from the inner forearms 24 young
  (20 to 25 years) and 24 old (55 to 66 years) human volunteers were analysed using Agilent
  Whole Human Genome Oligo Microarrays 8x60K V2.",
  contributors = c("Kuehne A", "Hildebrand J", "Soehle J", "Wenck H", "Terstegen L",
                   "Gallinat S", "Knott A", "Winnefeld M", "Zamboni N"),
  title = "An integrative metabolomics and transcriptomics study to identify metabolic
...
net <- build_net(kuehne_expr_filtered, cor_func = "spearman", n_threads = threads_to_use)
...
```{r modules_detection}
detection <- detect_modules(kuehne_expr_filtered, net$network, detailled_result = TRUE)
```
...
### 6. Graph visualization and topological analysis
...
```{r graph, fig.width=12, fig.height=8}
module_2 <- detection$modules$`2`
graph <- build_graph_from_sq_mat(net$network[module_2, module_2])
plot_module(graph, weight_th = 0.999995, vertex.label.cex = 0, node_scaling_max = 7, legend_cex = 1)
```
...
se_expr_by_cond <- sapply(unique(se_kuehne$Condition), function(cond){
     se_kuehne[, se_kuehne$Condition == cond]
})

# Network building and modules detection by condition
net_by_cond <- lapply(expr_by_cond, build_net, cor_func = "spearman", n_threads = threads_to_use, keep_matrices = "both")
...
The final object contains a table summarizing the comparison of the modules, directly available with the `comparison$result$young$old$comparison` command.
